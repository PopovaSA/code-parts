...
Перед текстом, который надо перенести, ставится следующая комбинация символов (!без пробелов): 
< br >
...
### 5.6	Оформить ссылку как кнопку
```javascript
TextRender({
    text: w.general,
    style: w.style
...
### 5.7	Подтянуть в текстовый виджет значение из фильтра
